<?php
declare(strict_types=1);

use Phinx\Config\FeatureFlags;
use Phinx\Migration\AbstractMigration;

final class CreateArbetyWebhook extends AbstractMigration
{
    /**
     * Change Method.
     *
     * Write your reversible migrations using this method.
     *
     * More information on writing migrations is available here:
     * https://book.cakephp.org/phinx/0/en/migrations.html#the-change-method
     *
     * Remember to call "create()" or "update()" and NOT "save()" when working
     * with the Table class.
     */
    public function change(): void
    {
        FeatureFlags::setFlagsFromConfig(['unsigned_primary_keys' => false]);

        $this->output->writeln('<info>Tabela</info> arbety_webhook');

        $table = $this->table('arbety_webhook')
            ->addColumn('tipo', 'enum', ['values' => ['MENSAL','TRIMESTRAL','SEMESTRAL','ANUAL'], 'default' => 'MENSAL', 'null' => false])
            ->addColumn('id_webhook', 'string', ['null' => false])
            ->addColumn('plataforma', 'enum', ['values' => ['LASTLINK'], 'default' => 'LASTLINK', 'null' => false])
            ->addColumn('produto', 'string', ['null' => false])
            ->addColumn('evento', 'string', ['null' => false])
            ->addColumn('email', 'string', ['null' => false])
            // ->addTimestamps('create_at', false)
            ->addColumn('create_at', 'datetime')
            ->addColumn('id_usuario', 'integer')
            ->addForeignKey('id_usuario', 'arbety_user')
            ->addIndex(['id_usuario'])
            ->addIndex(['email'])
            ->create();
    }
}
