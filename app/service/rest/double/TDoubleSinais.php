...
class TDoubleSinais
{
    ...
    public static function iniciar($param)
    {
        ...
        if ($data->plataforma->tipo_sinais == 'PROPAGA_OUTRO')
    ...
    {
        $data = unserialize(base64_decode($param['data']));
                    ...
                    foreach ($estrategias as $key => $estrategia) {
                        if ($estrategia->validar($sinal, $service)) {
                            ...
                            $telegram->sendMessage(
                                ...
                                str_replace(
                                    ['{estrategia}', '{cor}'],
                                    [$estrategia->nome, self::getCor($cor_retornada, $data->plataforma->translate)],
                ...
                try {
                    ...
                    if ($historico['tipo'] == 'POSSIVEL') {
                        ...
                        $telegram->sendMessage(
                            ...
                            str_replace(
                                ['{estrategia}', '{cor}'],
                                [$historico['estrategia'] ? $historico['estrategia']->nome : '', self::getCor($cor_retornada, $data->plataforma->translate)],
                                $data->plataforma->translate->MSG_SINAIS_ENTRADA_CONFIRMADA,
                            ),
                            [
                                "resize_keyboard" => true, 
    ...
    {
        // DoubleErros::registrar(1, 'TDoubleSinais', 'executar_usuario', '1');
        $data = unserialize(base64_decode($param['data']));
        if (!isset($data->tipo) or (isset($data->tipo) and $data->tipo != 'cmd'))
            throw new Exception($data->plataforma->translate->MSG_OPERACAO_METODO_NAO_SUPORTADO);
    ...
    public static function getCor($cor, TDoubletranslate $translate) {
        switch ($cor) {
            case 'red':
                $cor_result = $translate->COLOR_RED;
                break;
            case 'black':
                $cor_result = $translate->COLOR_BLACK;
                break;
            case 'lo':
                $cor_result = 'LO';
                break;
            ...
            default:
                $cor_result = $translate->COLOR_WHITE;
                break;
        }
