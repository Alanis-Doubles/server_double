...
class LoginForm extends TPage
{
    ...
    public static function onLogin($param)
    {
        ...
        try
        {
            ...
            if ($user)
            {
                ...
                if ( ($form = self::checkMultiUnit($user, $param)) instanceof BootstrapFormBuilder)
                {
                    new TInputDialog(_t('Select unit'), $form);
                    return;
                }
    ...
    public static function checkMultiUnit($user, $param) 
    {
        ...
        {
            $action = new TAction(['LoginForm', 'onLogin'], $param);
            $form = new BootstrapFormBuilder('multi_unit_form');
            ...
            $unit_id->setSize('70%');
            $unit_id->style = 'height:35px;font-size:14px;float:left;border-bottom-left-radius: 0;border-top-left-radius: 0;';
            $row = $form->addFields( [$unit, $unit_id] );
            $row->layout = ['col-sm-12 display-flex'];

            $btn = $form->addAction( _t('Select'), $action, '');
            $btn->class = 'btn btn-primary';
            $btn->style = 'height: 40px;width: 90%;display: block;margin: auto;font-size: 17px;';
            TCombo::reload('multi_unit_form', 'unit_id', $options);

            return $form;
        }
    }

    public static function changeUnit() 
    {
        $options = [];
        try
        {
        ...
        if (count($options) > 1)
        {
            $action = new TAction(['LoginForm', 'onChangeUnit']);
            $form = new BootstrapFormBuilder('multi_unit_form');
    ...
    public static function reloadPermissions()
    {
        try
        {
            ...
            $lang_id = TSession::getValue('user_language');

            if ($user)
            {
                ApplicationAuthenticationService::loadSessionVars($user);
                ApplicationAuthenticationService::setUnit( $unit_id ?? null );
                ApplicationAuthenticationService::setLang( $lang_id ?? null );
